...
- **스코프 제외**: `live_*` 폴더(대회 실전 풀이)는 감사하지 않는다 — 대회 종료 순간의 박제라 사용자가 절대 안 고침(CONVENTIONS.md §6.1). 컴파일 확인·스윕·산문 전부 건너뛴다. 학습·재구현은 짝이 되는 `cofo_*` 폴더에서 이뤄지고 그건 정상 감사 대상.
...
- 애매하면 백지 리뷰 서브에이전트에 `CONVENTIONS.md`만 쥐여주고 콜드 감사 위임 가능(사용자 승인). 콜드 대조로 blind spot이 실측된 전례 있음(배열우선 반사, `std::erase` 반사). 모델은 Sonnet으로 충분(2026-09-04 전수 재감사에서 검증).
- **재지적 금지 리스트 = `CONVENTIONS.md`의 `↔ 금지` 태그 + `[통용]`/`[취향]` 항목.** 시작 전에 이걸 먼저 읽고 서브에이전트 브리프에도 넣는다. (지난 감사에서 의식적 선택을 오지적한 전례 다수 — 철회율 높았음.)

## 대규모 재감사 (여러 날짜 한 번에)

한 날짜가 아니라 구간 전체(예: "day_20 이후 전부") 재점검이면:

1. **Phase 1 (부모, 일괄)** — 전 파일 컴파일/파싱 + 기계 스윕(아래 표)을 구간 전체에 한 번에 돌린다. 저하 위험 없음.
2. **Phase 2 (서브에이전트 병렬)** — 날짜 2개씩 묶어 Sonnet 서브에이전트에 배정(`run_in_background`). 각자 `CONVENTIONS.md` 전문 + 공통 브리프(특히 볼 규칙 + 재지적 금지 리스트 + Phase 1 히트) + 담당 폴더 목록. 신선한 컨텍스트라 "후반으로 갈수록 대충" 원천 차단.
3. **Phase 3 (부모)** — findings를 메모리 대조로 필터, "의도 vs 실수" 판단, 등급, 폴더 오름차순 문제별 산문. **날짜별 스트리밍 보고**.
4. **findings의 상당수가 코드가 아니라 규칙의 공백/오류일 수 있다** — 그땐 스윕표가 아니라 `CONVENTIONS.md`를 고친다 (2026-09-04 재감사: §7.1 레벨BFS·§6.2 스택·§7.5 예시·§5.2·§6.3 카브아웃 등 다수 정정).
...
| Python C 전사            | `while .+\* .+<=` · `i ?\+= ?1` · `range\(len\(` · `for \w+ in range\([se], ` | `isqrt`+`for` / `enumerate` / 슬라이스 컴프리헨션. `range(s, e+1)`로 `arr[j]`만 읽으면 🟡 (§5.3)           |
| Python import            | `^(import\|from) `                                                            | `CONVENTIONS.md` §5.3 (`math`/`sys`/`heapq` bare, 나머지 from)                                             |
| Java 토큰화              | `\.split\(" "\)`                                                              | 기본은 `StringTokenizer` (§5.2). split은 곧장 순회/위치접근이 확연히 깔끔할 때만, `arr[i-1]` 재파싱 X (🟡) |
| 자리표시자 이름          | `\b(flag\|func\|lst)\b`                                                       | §3.4·§3.5 (🟡)                                                                                             |
| 자릿수 · gcd 변수        | `% ?10` · `= *(gcd\|__gcd\|math\.gcd\|std::gcd)\(`                            | 자릿수는 `d`, gcd 결과는 `g` (`d`=자릿수와 충돌) — 벗어나면 🟡 (§3.1)                                      |
...
패턴은 시작점 — `CONVENTIONS.md` §3~§7에 규칙이 추가되면 스윕 항목도 같이 늘린다. **단 실측분만** (이 감사에서 실제로 걸린 규칙만; 미발생 위반 방어 패턴 선등록 X).
...
- 새 규칙이거나 **기존 규칙이 틀렸/불완전했으면** `CONVENTIONS.md`(정본)를 갱신하고, 실측 패턴이면 위 스윕 표에도 추가.
- day_XX가 실제 알고리즘 콘텐츠(그래프·DP·재귀 등)를 담고 있으면 새 패턴 확립 기회 — `CONVENTIONS.md` §7 확장 여지 확인.
- 메모리 `ps_style_rationale`·`ps_language_style_guide`에는 규칙을 안 담는다(정본은 `CONVENTIONS.md`). 사용자 원문·외부 근거만.
